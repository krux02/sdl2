...
template SDL_FOURCC (a,b,c,d: char): uint32 =
  (
    (a.uint8.uint32 shl 0)  or 
    (b.uint8.uint32 shl 8)  or 
    (c.uint8.uint32 shl 16) or 
    (d.uint8.uint32 shl 24))

template SDL_DEFINE_PIXELFOURCC*(A, B, C, D: char): uint32 =
  SDL_FOURCC(A, B, C, D)

template SDL_DEFINE_PIXELFORMAT*(`type` , order , layout, bits, bytes: uint32): uint32 =
  ((1 shl 28) or ((`type`) shl 24) or ((order) shl 20) or ((layout) shl 16) or
      ((bits) shl 8) or ((bytes) shl 0))

template SDL_PIXELFLAG*(X: uint32): uint32 =
  (((X) shr 28) and 0x0000000F)

template SDL_PIXELTYPE*(X: uint32): uint32 =
  (((X) shr 24) and 0x0000000F)

template SDL_PIXELORDER*(X: uint32): uint32 =
  (((X) shr 20) and 0x0000000F)

template SDL_PIXELLAYOUT*(X: uint32): uint32 =
  (((X) shr 16) and 0x0000000F)

template SDL_BITSPERPIXEL*(X: uint32): uint32 =
  (((X) shr 8) and 0x000000FF)

template SDL_BYTESPERPIXEL*(X: uint32): uint32 =
...
template SDL_ISPIXELFORMAT_INDEXED*(format: uint32): uint32 =
...
template SDL_ISPIXELFORMAT_ALPHA*(format: uint32): uint32 =
  (not SDL_ISPIXELFORMAT_FOURCC(format) and
